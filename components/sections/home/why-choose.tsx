        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item, index) => (
            <Reveal key={item.title} delay={index * 90} className="h-full">
              <WhyChooseCard number={index + 1} icon={item.icon} title={item.title} description={item.description} />
            </Reveal>
          ))}
        </div>
      </div>
...
function WhyChooseCard({ number, icon: Icon, title, description }: WhyChooseCardProps) {
  return (
    <div className="group relative flex h-full flex-col justify-between gap-8 overflow-hidden border border-border bg-muted p-7 transition-colors duration-300 hover:bg-muted/70 lg:aspect-square">
      ...
      <div className="flex items-start justify-between">
        <Icon className="size-7 text-primary transition-colors duration-300 group-hover:text-accent" />
